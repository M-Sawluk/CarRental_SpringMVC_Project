...
public class ProductRepositoryImpl implements ProductRepository {
	...
	public void setCarUnits(CarStorageForm cSF) {
		...
		for (int i = 0; i < cSF.getStorages().size(); i++) {
			...
			CarStorage cs = (CarStorage) sessionFactory.getCurrentSession()
					.createQuery(
					...
					.setParameter("id", cSF.getStorages().get(i).getCar().getId())
					.setParameter("pId", cSF.getStorages().get(i).getRentingPlace().getId()).uniqueResult();
